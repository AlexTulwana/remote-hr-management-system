...
import { getAllEmployeeRequests, hrDecideRequest } from '../../api/employeeRequests';
...
function RequestInfo({ r }) {
  const branch = r.employee?.branchName || 'No branch';
  const position = r.employee?.position ? ` · ${r.employee.position}` : '';
  return (
    <div>
      ...
      {r.hrComment ? <p className="text-[12px] text-text-secondary mt-1">HR: {r.hrComment}</p> : null}
    </div>
  );
}
...
export default function HrEmployeeRequests() {
  ...
  async function handleConfirm(comment) {
    ...
    setActionError('');
    try {
      await hrDecideRequest(target.request.id, target.decision, comment || null);
      setTarget(null);
      setReloadKey((k) => k + 1);
    } catch (err) {
  ...
  const byOldest = (a, b) => new Date(a.submittedAt) - new Date(b.submittedAt);
  const byNewest = (a, b) => new Date(b.submittedAt) - new Date(a.submittedAt);

  const needsDecision = requests
    .filter((r) => r.status === 'ESCALATED' || (r.status === 'PENDING' && !r.employee?.branchName))
    .sort(byOldest);
  const waitingOnManager = requests
    .filter((r) => r.status === 'PENDING' && r.employee?.branchName)
    .sort(byOldest);
  ...
  return (
    <div>
      <p className="text-[20px] font-medium mb-5">Employee Requests</p>
      ...
      {loading ? (
        ...
        <>
          <p className="text-[13px] font-medium text-text-secondary mb-2">Needs your decision</p>
          <Card className="mb-5">
            {needsDecision.length === 0 ? (
              <EmptyState message="Nothing waiting for an HR decision." />
            ) : (
          ...
          <p className="text-[13px] font-medium text-text-secondary mb-2">Waiting on a manager</p>
          <Card className="mb-5">
            {waitingOnManager.length === 0 ? (
              <EmptyState message="No requests waiting on a manager." />
            ) : (
              <div className="flex flex-col divide-y divide-border">
                {waitingOnManager.map((r) => (
