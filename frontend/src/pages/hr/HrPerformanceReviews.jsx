...
export default function HrPerformanceReviews() {
  const { user } = useAuth();
  ...
  const branches = [...new Set(directory.map((m) => m.branchName).filter(Boolean))].sort();
  ...
  const employeeBranch = new Map(directory.map((m) => [String(m.id), m.branchName]));

  const visible = reviews
    .filter((r) => !filterEmployeeId || String(r.employee?.id) === filterEmployeeId)
    .filter((r) => !filterBranch || employeeBranch.get(String(r.employee?.id)) === filterBranch)
    .sort((a, b) => {
  ...
  return (
    <div>
            ...
            <Select label="Employee" value={formEmployeeId} onChange={(e) => setFormEmployeeId(e.target.value)}>
              ...
              {candidates.map((m) => (
                <option key={m.id} value={m.id}>
                  {m.fullName} {m.branchName ? `(${m.branchName})` : ''}
                </option>
              ))}
            </Select>
      ...
      {(branches.length > 0 || reviewed.size > 0) ? (
        <Card className="mb-3.5">
          <div className="grid grid-cols-2 gap-3">
              ...
              <div key={r.id} className="flex items-start justify-between gap-3 py-3 first:pt-0 last:pb-0">
                <div className="min-w-0">
                  <p className="text-[13px] font-medium mb-0.5">
                    {r.employee?.fullName}
                    {r.employee?.branchName ? (
                      <span className="text-text-secondary font-normal"> · {r.employee.branchName}</span>
                    ) : null}
                  </p>
